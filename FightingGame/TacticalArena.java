...
public class TacticalArena extends JFrame {
    ...
    public TacticalArena() {
        ...
        player = new Combatant(L10n.PLAYER_NAME, 300);
    ...
    private void setupUI() {
        ...
        gamePanel.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) { handleMouse(e.getPoint()); repaint(); }
        });
    ...
    private void handleMouse(Point p) {
        if (step == 0) {
            for (BodyPart bp : ai.parts.values()) if (bp.bounds.contains(p)) {
                resetVisuals();
                player.attackTarget = bp.type; step = 1; shortLog.setText(L10n.STEP_2_EVA); return;
            }
        } else {
            for (BodyPart bp : player.parts.values()) if (bp.bounds.contains(p)) {
                if (step == 1) { player.evasionPoint = bp.type; step = 2; shortLog.setText(L10n.STEP_3_DEF); }
                else if (step == 2) { player.defensePoints.add(bp.type); step = 3; shortLog.setText(L10n.STEP_4_DEF); }
    ...
    private void runTurn() {
        ...
        while(ai.defensePoints.size() < 2) ai.defensePoints.add(BodyPartType.values()[(int)(Math.random()*7)]);
    ...
    private void saveChoices() {
        player.lastAtk = player.attackTarget; player.lastEva = player.evasionPoint; player.lastDef.addAll(player.defensePoints);
        ai.lastEva = ai.evasionPoint; ai.lastDef.addAll(ai.defensePoints);
    ...
    private void drawCombatant(Graphics2D g, Combatant c, boolean isPlayerSide) {
        ...
        for (BodyPart part : c.parts.values()) {
            ...
            g.setColor(new Color(55, 58, 65)); g.fill(shape);
            ...
            g.fill(shape); g.setClip(null);
            g.setColor(new Color(100, 105, 115)); g.draw(shape);
