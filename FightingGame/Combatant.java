// ==========================================
// 4. МОДЕЛЬ ПЕРСОНАЖА (ЛОГІКА ХАРАКТЕРИСТИК)
// ==========================================
...
class Combatant {
    ...
    public float getArmEff() { return (parts.get(BodyPartType.L_ARM).hp + parts.get(BodyPartType.R_ARM).hp) / 200f; }
    public float getLegEff() { return (parts.get(BodyPartType.L_LEG).hp + parts.get(BodyPartType.R_LEG).hp) / 200f; }
    public float getHeadEff() { return parts.get(BodyPartType.HEAD).hp / 100f; }
    public float getBleedSum() { return (float) parts.values().stream().mapToDouble(p -> p.bleeding).sum(); }
    public float getPower() { return getArmEff() * 100f; }
    public float getEvasionChance() { return GameSettings.BASE_EVASION * getLegEff(); }
    public float getAccuracy() { 
        float armDebuff = (1.0f - getArmEff()) * 0.5f;
        float headDebuff = 1.0f - getHeadEff();
        return GameSettings.BASE_ACCURACY - armDebuff - headDebuff;
    }
}