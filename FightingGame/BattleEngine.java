...
public class BattleEngine {
    ...
    public String processAttack(Combatant atk, Combatant def, boolean isPlayerAtk, JTextArea log) {
        ...
        if (bleedAdded > 0) log.append(String.format(L10n.CONS_BLEED, def.name, bleedAdded));
    ...
    private void logConsequence(JTextArea log, String name, String pattern, float before, float after, int mult) {
        if (pattern != null && before > after + 0.001) log.append(String.format(pattern, name, (before - after) * mult));
    }

    public void applyBleeding(Combatant c, JTextArea log) {
        float actual = Math.min(c.getBleedSum(), c.globalHp * 0.10f);
        if (actual > 0.5) {
            c.globalHp -= actual;
